import { INDataCenter, OAuthBuilder, InitializeBuilder } from "@zohocrm/nodejs-sdk-8.0";
...
const initializeZoho = async () => {
  ...
  await (await new InitializeBuilder()).environment(environment).token(token).initialize();

  isInitialized = true;
  console.log("Zoho SDK initialized");
};

export { initializeZoho };
